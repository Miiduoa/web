...
BEGIN
  ...
  ON CONFLICT (user_id) DO UPDATE SET
    changed_at=excluded.changed_at,attempts=0,next_attempt_at=now(),last_error='',revision=excluded.revision;
...
BEGIN
  ...
  FOR v_req IN
    SELECT request_id,user_id,revision,created_at FROM public.puplan_replication_requests ORDER BY created_at LIMIT 100
  LOOP
    ...
    IF coalesce(v_found,false) THEN
      ...
      ELSE
        ...
        SET attempts=attempts+1,next_attempt_at=now(),
            last_error=left(coalesce(nullif(v_error,''),'HTTP '||coalesce(v_status::text,'unknown')),500)
    ...
    ELSIF v_req.created_at<now()-interval '30 seconds' THEN
      UPDATE public.puplan_replication_outbox
      SET attempts=attempts+1,next_attempt_at=now(),last_error='replication acknowledgement timed out'
      WHERE user_id=v_req.user_id AND revision=v_req.revision;
      DELETE FROM public.puplan_replication_requests WHERE request_id=v_req.request_id;
      v_failed:=v_failed+1;
    END IF;
  ...
  FOR v_out IN
    ...
    WHERE o.next_attempt_at<=now()
      AND NOT EXISTS (SELECT 1 FROM public.puplan_replication_requests r WHERE r.user_id=o.user_id AND r.revision=o.revision)
  ...
  SET last_success_at=CASE WHEN v_failed=0 THEN now() ELSE last_success_at END,
      ...
      pending_count=v_pending,updated_at=now()
  WHERE id=1;
...
-- Existing accounts become eligible for standby without needing another browser login.
...
ON CONFLICT (user_id) DO UPDATE SET
  changed_at=excluded.changed_at,attempts=0,next_attempt_at=now(),last_error='backfill',
  revision=greatest(public.puplan_replication_outbox.revision,excluded.revision);
