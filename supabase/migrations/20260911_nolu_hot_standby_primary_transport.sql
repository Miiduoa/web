...
BEGIN
  ...
  ON CONFLICT (user_id) DO UPDATE SET
    revision=public.puplan_replication_versions.revision+1,
    updated_at=now()
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(jsonb_build_object(
    ...
    'school',s.school,'department',s.department,'class_name',s.class_name,
    'credits',s.credits,'courses',s.courses,'is_current',s.is_current,
  ...
  RETURN jsonb_build_object(
    ...
    'iat',v_now,'exp',v_now+interval '2 minutes','nonce',gen_random_uuid(),
    'uid',p_uid,'revision',p_revision,
    'bundle',jsonb_build_object('user',v_user,'schedule',v_schedule,'semesters',v_semesters)
  );
END;
$$;
