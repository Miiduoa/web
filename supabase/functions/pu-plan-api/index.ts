...
Deno.serve(async (req:Request)=>{
  ...
  try{
    ...
    if(action==='save_schedule'){ const courses=validateCourses(body.courses); const key=cleanText(body.semester_key,24); let target:any=null; if(key){ const {data}=await db.from('puplan_app_semesters').select('id,is_current').eq('user_id',user.id).eq('semester_key',key).maybeSingle(); target=data; } if(!target){ const {active}=await ensureCurrentSemester(user.id); target=active; } const {error}=await db.from('puplan_app_semesters').update({courses,updated_at:new Date().toISOString()}).eq('id',target.id).eq('user_id',user.id); if(error)throw error; if(target.is_current!==false)await mirrorLegacy(user.id,courses); return json({ok:true}); }
