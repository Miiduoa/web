...
async function verifyEnvelope(raw: string) {
  ...
  if (!payloadPart || !signaturePart || extra.length) throw new ApiError(401, 'invalid replica signature', 'REPLICA_SIGNATURE_INVALID');
  let verified = false;
  try { verified = ed25519.verify(fromB64url(signaturePart), encoder.encode(payloadPart), fromB64url(PRIMARY_PUBLIC_KEY)); } catch { verified = false; }
  if (!verified) throw new ApiError(401, 'invalid replica signature', 'REPLICA_SIGNATURE_INVALID');
  let payload: any;
  try { payload = JSON.parse(decoder.decode(fromB64url(payloadPart))); } catch { throw new ApiError(400, 'invalid replica payload', 'REPLICA_PAYLOAD_INVALID'); }
  const now = Date.now(), iat = Date.parse(String(payload.iat || '')), exp = Date.parse(String(payload.exp || '')), revision = Number(payload.revision);
  if (payload?.v !== 2 || payload.iss !== PRIMARY_REF || payload.aud !== STANDBY_REF || !['core-seed', 'core-delete'].includes(String(payload.kind || '')) || !UUID.test(String(payload.uid || '')) || !UUID.test(String(payload.nonce || '')) || !Number.isSafeInteger(revision) || revision <= 0 || !Number.isFinite(iat) || !Number.isFinite(exp) || exp <= now || iat > now + 30000 || exp - iat > 180000 || now - iat > 180000) throw new ApiError(401, 'replica envelope expired or invalid', 'REPLICA_ENVELOPE_INVALID');
  return payload;
}
