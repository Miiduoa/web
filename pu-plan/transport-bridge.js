...
function freshOptions(options={}){
  if(!hasAuth(options))return options;
  const token=localStorage.getItem('puplan_session')||'';if(!token)return options;
...
function ordered(action=''){
  const prefer=localStorage.getItem('nolu_preferred_cloud_v1')==='standby';
  const cloud=prefer?[STANDBY,PRIMARY]:[PRIMARY,STANDBY];
...
async function timed(url,options={},ms=1800){
  ...
  try{return await baseFetch(url,{...freshOptions(options),signal:ctrl.signal,cache:'no-store'})}
...
window.fetch=async function noluLegacyFailover(input,options={}){
  ...
  const action=String(bodyOf(options)?.action||'');
  let lastResponse=null,lastError=null,standby401=false;
  const candidates=ordered(action).filter(endpoint=>!isOpen(endpoint));
  if(!candidates.length)throw new TypeError('Nolu transport temporarily unavailable');
  for(const endpoint of candidates){
    ...
    try{
      ...
      if(response.status<500){success(endpoint);if(endpoint===STANDBY)localStorage.setItem('nolu_preferred_cloud_v1','standby');return response}
