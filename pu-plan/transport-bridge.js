...
function health(endpoint){
  if(!state.endpoints[endpoint])state.endpoints[endpoint]={failures:0,openUntil:0,lastFailureAt:0,lastSuccessAt:0,lastError:''};
...
window.fetch=async function noluLegacyFailover(input,options={}){
  ...
  const candidates=CANDIDATES.filter(endpoint=>!isOpen(endpoint));
