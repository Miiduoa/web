...
// v7 and the first cross-project sync experiment are intentionally retired by
// the security guardrail. Keep their URLs recognizable so an older cached PWA
// request is intercepted, but never select them as a live transport candidate.
...
const RETIRED_PRIMARY='https://hrrmkrayvrgnwcroyttp.supabase.co/functions/v1/pu-plan-api-v7';
const RETIRED_STANDBY='https://ltfurqaspqsvswmebyzw.supabase.co/functions/v1/pu-plan-api-v7';
const CORE_APIS=[PRIMARY,FALLBACK,RETIRED_PRIMARY,RETIRED_STANDBY];
const nativeFetch=window.fetch.bind(window);
const VERSION='20260911-retired-ha-compat1';
...
function isRetired(api){return api===RETIRED_PRIMARY||api===RETIRED_STANDBY}
...
function preferredCloud(){return'primary'}
function setPreferredCloud(){localStorage.removeItem(PREFERRED_KEY)}
function setMode(mode,api=''){
  state.mode=mode;if(api){state.activeApi=api;state.tier=api.includes('ltfurqaspqsvswmebyzw')?'secondary':'primary'}if(mode==='online'){state.lastOnlineAt=now();state.lastError=''}
...
function allApiCandidates(requested){
  const extra=Array.isArray(window.NOLU_SECONDARY_APIS)?window.NOLU_SECONDARY_APIS.filter(x=>typeof x==='string'&&/^https:\/\//.test(x)&&!isRetired(x)):[];
  const requestedSafe=isRetired(String(requested))?'':String(requested||'');
  return [...new Set([PRIMARY,FALLBACK,requestedSafe,...extra].filter(Boolean))];
}
function apiCandidates(requested,{force=false}={}){const all=allApiCandidates(requested);return force?all:all.filter(api=>!circuitOpen(api))}
function earliestProbe(requested){const all=allApiCandidates(requested);if(!all.length)return'';return [...all].sort((a,b)=>(endpointState(a).openUntil||0)-(endpointState(b).openUntil||0))[0]}
...
async function timedFetch(url,options={},ms=2400){
  ...
  try{return await nativeFetch(url,{...options,signal:ctrl.signal,cache:'no-store'})}finally{clearTimeout(t);if(outer)outer.removeEventListener('abort',abort)}
}
...
async function captureSuccess(res,body,api,sequence){
  if(!res?.ok)return;state.activeApi=api;if(sequence>=newestOfflineSequence)setMode('online',api);
  try{const data=await res.clone().json();const uid=data?.profile?.id||currentUid();if(data?.profile?.id){localStorage.setItem('puplan_course_owner',String(data.profile.id));snapshot(String(data.profile.id),data.profile)}if(uid&&body?.action)clearMatchingPending(uid,body.action,body)}catch{}
}
...
window.fetch=async function noluResilientFetch(input,options={}){
  ...
  const sequence=++requestSequence,body=requestBody(options),action=String(body?.action||'');let lastError=null,lastResponse=null;
  const candidates=apiCandidates(String(url));if(!candidates.length){state.lastError='all cloud circuits are temporarily open';const local=offlineResponse(action,body,sequence);if(local)return local;throw new TypeError('Nolu cloud temporarily unavailable')}
...
async function directJson(api,action,payload={},auth=true,ms=5000){
  if(isRetired(api))return {res:new Response(JSON.stringify({error:'DISABLED'}),{status:410,headers:{'Content-Type':'application/json'}}),data:{error:'DISABLED'}};
  const headers={'Content-Type':'application/json'};if(auth&&token())headers.Authorization=`Bearer ${token()}`;
  try{const res=await timedFetch(api,{method:'POST',headers,body:JSON.stringify({action,...payload}),cache:'no-store'},ms);if(res.status<500)markSuccess(api);else markFailure(api,new Error(`HTTP ${res.status}`));const data=await res.json().catch(()=>({}));return {res,data}}catch(error){markFailure(api,error);throw error}
}
async function flushPending(api){
  const uid=currentUid();if(!uid||isRetired(api))return false;let q=readPending(uid);
...
async function recover(){
  ...
  try{
    let candidates=apiCandidates(PRIMARY);if(!candidates.length){const probe=earliestProbe(PRIMARY);candidates=probe?[probe]:[]}
    for(const api of candidates){
      try{const {res}=await directJson(api,'bootstrap',{},true,3600);if(res.ok){if(await flushPending(api)){setMode('online',api);sessionStorage.setItem('puplan_api_index',String(api===FALLBACK?1:0));location.reload()}return}if(res.status===401){document.querySelector('#authGate')?.classList.remove('off');return}}catch{}
    }
...
(function init(){
  // Retire client state created by the insecure v7/sync experiment. A future
  // standby must obtain its own independently verifiable session.
  localStorage.removeItem(PORTABLE_KEY);localStorage.removeItem(PREFERRED_KEY);
...
window.NOLU_RESILIENCE={state,getMode:()=>state.mode,recover,flushPending,snapshot,currentUid,circuitOpen,apiCandidates,preferredCloud,setPreferredCloud,PRIMARY,FALLBACK,RETIRED_PRIMARY,RETIRED_STANDBY};
