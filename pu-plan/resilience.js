...
function setPreferredCloud(value){
  if(value==='standby'||value==='primary')localStorage.setItem(PREFERRED_KEY,value);
}
function setMode(mode,api=''){
  state.mode=mode;
  if(api){state.activeApi=api;state.tier=api===STANDBY?'standby':'primary'}
  if(mode==='online'){state.lastOnlineAt=now();state.lastError=''}
  document.documentElement.dataset.noluConnectivity=mode;
  document.documentElement.dataset.noluCloudTier=state.tier;
...
function allApiCandidates(requested,action=''){
  ...
  const list=[...base,requested,COMPAT_V6,COMPAT_CORE,...extra].filter(Boolean);
  return [...new Set(list)].filter(api=>!(api===STANDBY&&NO_STANDBY_ACTIONS.has(action)));
}
...
function freshOptions(options={}){
  if(!hasAuthorization(options)||!token())return options;
  const headers=new Headers(options.headers||{});headers.set('Authorization',`Bearer ${token()}`);return {...options,headers};
}
async function timedFetch(url,options={},ms=1800){
  ...
  try{return await nativeFetch(url,{...freshOptions(options),signal:ctrl.signal,cache:'no-store'})}
...
function offlineResponse(action,body,sequence){
  if(action==='bootstrap'){newestOfflineSequence=Math.max(newestOfflineSequence,sequence);return syntheticBootstrap()}
  if(action==='save_schedule'||action==='update_profile'){newestOfflineSequence=Math.max(newestOfflineSequence,sequence);return syntheticMutation(action,body)}
  return null;
}
function clearMatchingPending(uid,action,body){
  const q=readPending(uid);
  if(action==='save_schedule'&&q.schedule){const sent=Array.isArray(body?.courses)?body.courses.slice(0,80):[];if(sameJson(q.schedule.courses,sent)){delete q.schedule;writePending(uid,q)}}
  if(action==='update_profile'&&q.profile&&sameJson(q.profile.payload,body)){delete q.profile;writePending(uid,q)}
}
async function captureSuccess(res,body,api,sequence){
  if(!res?.ok)return;
  state.activeApi=api;if(api===STANDBY)setPreferredCloud('standby');
  if(sequence>=newestOfflineSequence)setMode('online',api);
  try{
    const data=await res.clone().json();
    if(typeof data?.token==='string'&&data.token.split('.').length===2&&data.token!==token()){
      localStorage.setItem('puplan_session',data.token);
...
window.fetch=async function noluResilientFetch(input,options={}){
  ...
  const sequence=++requestSequence,body=requestBody(options),action=String(body?.action||'');
  let lastError=null,lastResponse=null,standby401=null;
  const candidates=apiCandidates(String(url),{action});
  if(!candidates.length){state.lastError='all cloud circuits are temporarily open';const local=offlineResponse(action,body,sequence);if(local)return local;throw new TypeError('Nolu cloud temporarily unavailable')}
  for(const api of candidates){
    if(options?.signal?.aborted)break;
    try{
      const res=await timedFetch(api,options,1800);lastResponse=res;
      if(api===STANDBY&&res.status===401&&hasAuthorization(options)){
        standby401=res;lastError=new Error('standby authorization unavailable');markFailure(api,lastError);continue;
      }
...
async function directJson(api,action,payload={},auth=true,ms=3600){
  ...
  try{
    ...
    if(res.ok&&typeof data?.token==='string'&&data.token!==token()){localStorage.setItem('puplan_session',data.token);document.dispatchEvent(new CustomEvent('nolu:session-rotated',{detail:{api}}))}
    return {res,data};
...
async function flushPending(api){
  ...
  q=readPending(uid);
  if(q.schedule?.courses){const sent=q.schedule,{res}=await directJson(api,'save_schedule',{courses:sent.courses},true);if(!res.ok)return false;const latest=readPending(uid);if(latest.schedule?.changedAt===sent.changedAt&&sameJson(latest.schedule.courses,sent.courses)){delete latest.schedule;writePending(uid,latest)}}
  ...
  try{
    ...
    for(const api of candidates){
      try{
        ...
        if(res.ok){if(await flushPending(api)){if(api===STANDBY)setPreferredCloud('standby');setMode('online',api);sessionStorage.setItem('puplan_api_index','0');location.reload()}return}
