...
if(!app||!cloud)throw new Error('社群功能載入失敗');
...
function renderNowStrip(){
  ...
  if(current){const left=Math.max(0,courseEnd(current)-mins);first=`<div class="now-card hot"><span>現在</span><b>${esc(current.name)}</b><small>${periodLabel(current.start,current.end)} · ${esc(current.room||'')}</small><strong>${left} 分鐘後下課</strong></div>`}else{first=`<div class="now-card"><span>現在</span><b>目前沒在上課</b><small>${now.toLocaleTimeString('zh-TW',{hour:'2-digit',minute:'2-digit'})}</small><strong>${next?'先喘一下。':'今天可以收工了。'}</strong></div>`}
  if(next){const wait=courseStart(next)-mins;second=`<div class="now-card blue"><span>下一堂</span><b>${esc(next.name)}</b><small>${periodLabel(next.start,next.end)} · ${esc(next.room||'')}</small><strong>${fmtMins(wait)}後開始</strong></div>`}else if(future){second=`<div class="now-card blue"><span>接下來</span><b>${esc(future.course.name)}</b><small>${future.offset===1?'明天':dayName(future.day)} ${periodLabel(future.course.start,future.course.end)}</small><strong>${esc(building(future.course.room)||'記得看教室')}</strong></div>`}else{second=`<div class="now-card blue"><span>接下來</span><b>這週沒有下一堂</b><small>目前課表沒有其他課。</small><strong>先留白。</strong></div>`}
  const rest=day.filter(c=>courseStart(c)>mins).length;const same=sameBuildingMatches().length;third=`<div class="now-card lime"><span>今天</span><b>${day.length?`${day.length} 門課 · 還有 ${rest} 門`:'今天無課'}</b><small>${same?`有 ${same} 位好友今天跟你在同棟可能遇到。`:'今天暫時沒有同棟好友。'}</small><strong>${same?'去好友頁看看':'照自己的節奏走。'}</strong></div>`;
...
function renderMeetups(){return `<div class="meetups-view"><div class="social-feature-head"><div><h2>不要只看課表，直接約。</h2><p>共同空堂發出去後，對方可以直接接受或婉拒。</p></div></div><div class="meetup-stack">${renderMeetupCards()}</div></div>`}
...
function renderSocialSummary(){
  ...
  box.innerHTML=`<button class="social-stat" data-jump-tool="profile"><span>好友</span><b>${f.length}</b><small>已接受好友</small></button><button class="social-stat blue" data-jump-tool="same-building"><span>今天附近</span><b>${same.length}</b><small>今天可能遇到</small></button><button class="social-stat pink" data-jump-tool="meetups"><span>待回覆</span><b>${pending}</b><small>好友邀約</small></button>`;box.querySelectorAll('[data-jump-tool]').forEach(b=>b.onclick=()=>{friendTool=b.dataset.jumpTool;renderFriendTool();$('#friendView')?.scrollIntoView({behavior:'smooth',block:'start'})});
