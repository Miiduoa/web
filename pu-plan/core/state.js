...
const MEDIA_HOSTS=new Set(['hrrmkrayvrgnwcroyttp.supabase.co','ltfurqaspqsvswmebyzw.supabase.co']);
...
export function cleanMediaUrl(value){try{const u=new URL(String(value??''));if(u.protocol!=='https:'||u.username||u.password||u.port||!MEDIA_HOSTS.has(u.hostname))return'';if(!u.pathname.startsWith('/storage/v1/object/sign/puplan-media/'))return'';if(!u.searchParams.get('token'))return'';return u.toString()}catch{return''}}
