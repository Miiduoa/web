...
let token=localStorage.getItem('puplan_session')||'', profile=null, socialData={relationships:[],profiles:[],friends:[]}, searchTimer=null, syncTimer=null;
...
function updateAccountUI(){
  const name=profile?.display_name||'訪客'; if($('#accountName'))$('#accountName').textContent=name;if($('#accountAvatar'))$('#accountAvatar').textContent=initial(name);
  if($('#name'))$('#name').value=profile?.display_name||localStorage.getItem('puplan_name')||'';if($('#username'))$('#username').value=profile?.username||localStorage.getItem('puplan_username')||'';
...
function saveProfileLocal(p){profile=p;localStorage.setItem('puplan_name',p.display_name);localStorage.setItem('puplan_username',p.username);updateAccountUI();app?.renderShare?.()}
function clearSession(toast=true){token='';profile=null;socialData={relationships:[],profiles:[],friends:[]};localStorage.removeItem('puplan_session');const legacy=(app?.friends?.()||[]).filter(f=>!f.cloud);app?.setFriends?.(legacy);renderRequests();updateAccountUI();if(toast)app?.toast?.('已登出')}
function applySocial(social){socialData=social||{relationships:[],profiles:[],friends:[]};const cloud=Array.isArray(socialData.friends)?socialData.friends:[];const legacy=(app?.friends?.()||[]).filter(f=>!f.cloud);app?.setFriends?.([...cloud,...legacy]);renderRequests()}
function relationFor(id){return (socialData.relationships||[]).find(r=>r.requester_id===id||r.addressee_id===id)}
function profileFor(id){return (socialData.profiles||[]).find(p=>p.id===id)||{display_name:'使用者',username:''}}

async function bootstrap(){
  if(!token)return false;
  try{const data=await api('bootstrap');saveProfileLocal(data.profile);if(Array.isArray(data.courses))app?.setRemoteCourses?.(data.courses);applySocial(data.social);hideGate();return true}catch(e){console.warn(e);clearSession(false);return false}
}
async function login(email,password){
  const data=await api('login',{email,password},false);token=data.token;localStorage.setItem('puplan_session',token);localStorage.removeItem('puplan_guest');saveProfileLocal(data.profile);if(Array.isArray(data.courses))app?.setRemoteCourses?.(data.courses);applySocial(data.social);hideGate();return data;
}
async function signup(display_name,username,email,password){
  const localCourses=app?.courses?.()||[];const data=await api('signup',{display_name,username,email,password},false);token=data.token;localStorage.setItem('puplan_session',token);localStorage.removeItem('puplan_guest');saveProfileLocal(data.profile);applySocial(data.social);hideGate();if(localCourses.length){await api('save_schedule',{courses:localCourses});app?.setRemoteCourses?.(localCourses)}return data;
}
async function logout(){clearSession(true);localStorage.setItem('puplan_guest','1');hideGate()}
async function updateProfile(display_name,username){
  ...
  try{const data=await api('update_profile',{display_name:display_name.trim(),username});saveProfileLocal(data.profile);app?.toast?.('個人資料已更新')}catch(e){app?.toast?.(e.message)}
}
...
async function loadSocial(){if(!isSignedIn())return;try{const data=await api('social');applySocial(data.social)}catch(e){app?.toast?.(e.message)}}
...
function renderRequests(){
  ...
  incoming.forEach(r=>{const p=profileFor(r.requester_id);html+=`<div class="request-card"><div class="avatar">${esc(initial(p.display_name))}</div><div><b>${esc(p.display_name)}</b><small>@${esc(p.username)} · 想加你好友</small></div><div><button class="mini-btn" data-accept="${r.id}">接受</button><button class="mini-btn secondary" data-decline="${r.id}" style="margin-left:3px">略過</button></div></div>`});
  outgoing.forEach(r=>{const p=profileFor(r.addressee_id);html+=`<div class="request-card"><div class="avatar">${esc(initial(p.display_name))}</div><div><b>${esc(p.display_name)}</b><small>@${esc(p.username)} · 等待接受</small></div><button class="mini-btn secondary" data-cancel="${r.id}">取消</button></div>`});
...
window.PUPLAN_CLOUD={isSignedIn,showGate,logout,updateProfile,removeFriend,searchPeople,loadSocial,syncSchedule:()=>saveSchedule(app?.courses?.()||[])};
