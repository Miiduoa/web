...
const avatarOf=p=>cleanAvatar(p?.avatar_data||p?.avatar||'');
const avatarHTML=(p,cls='avatar')=>{const avatar=avatarOf(p),safeClass=String(cls||'avatar').replace(/[^A-Za-z0-9_-]/g,'')||'avatar';return avatar?`<div class="${safeClass}"><img src="${avatar}" alt=""></div>`:`<div class="${safeClass}">${esc(initial(p?.display_name||p?.name))}</div>`};
...
function purgeAccountCache(){
  for(const key of ['puplan_courses','puplan_friends','puplan_schedule_meta','puplan_course_owner','puplan_name','puplan_username','puplan_bio','puplan_avatar','puplan_discoverable'])localStorage.removeItem(key);
...
function isolateAccountCache(userId){
  ...
  if(owner!==userId)purgeAccountCache();
  localStorage.setItem('puplan_course_owner',userId);
}
function updateAccountUI(){
  const name=profile?.display_name||'訪客';
  const accountAvatar=avatarOf(profile);
  const previewAvatar=cleanAvatar(profile?.avatar_data||localStorage.getItem('puplan_avatar')||'');
  if($('#accountName'))$('#accountName').textContent=name;
  if($('#accountAvatar'))$('#accountAvatar').innerHTML=accountAvatar?`<img src="${accountAvatar}" alt="">`:esc(initial(name));
  ...
  if($('#profileAvatarPreview'))$('#profileAvatarPreview').innerHTML=previewAvatar?`<img src="${previewAvatar}" alt="">`:esc(initial(name));
...
function saveProfileLocal(p){
  const avatar=cleanAvatar(p?.avatar_data||'');
  profile={...p,avatar_data:avatar};localStorage.setItem('puplan_name',profile.display_name);localStorage.setItem('puplan_username',profile.username);localStorage.setItem('puplan_bio',profile.bio||'');
...
function renderRequests(){
  ...
  incoming.forEach(r=>{const p=profileFor(r.requester_id);html+=`<div class="request-card">${avatarHTML(p)}<div><b>${esc(p.display_name)}</b><small>@${esc(p.username)} · 想加你好友</small></div><div><button class="mini-btn" data-accept="${esc(r.id)}">接受</button><button class="mini-btn secondary" data-decline="${esc(r.id)}" style="margin-left:3px">略過</button></div></div>`});
  outgoing.forEach(r=>{const p=profileFor(r.addressee_id);html+=`<div class="request-card">${avatarHTML(p)}<div><b>${esc(p.display_name)}</b><small>@${esc(p.username)} · 等待接受</small></div><button class="mini-btn secondary" data-cancel="${esc(r.id)}">取消</button></div>`});
