...
function tokenFor(tier){
  if(tier==='standby')return localStorage.getItem('puplan_session_standby_v1')||'';
  return localStorage.getItem('puplan_session_primary_v1')||localStorage.getItem('puplan_session')||'';
}
function adminTarget(raw){
  try{
    const u=new URL(typeof raw==='string'||raw instanceof URL?String(raw):raw?.url||'',location.href);
    return u.hostname===`${PRIMARY_REF}.supabase.co`&&u.pathname==='/functions/v1/pu-plan-admin';
  }catch{return false}
}
async function call(url,options,tier){
  const token=tokenFor(tier);
  if(!token)return null;
  const headers=new Headers(options?.headers||{});
  headers.set('Authorization',`Bearer ${token}`);
  headers.set('Content-Type',headers.get('Content-Type')||'application/json');
  return nativeFetch(url,{...options,headers,cache:'no-store'});
}
...
window.fetch=async function noluAdminRegionalFailover(input,options={}){
  ...
  if(standby){
    if(standby.ok)localStorage.setItem('nolu_admin_cloud_v1','standby');
    return standby;
  }
