...
old = """let currentTab='feed',currentConversation='',inboxData=[],feedData=[],feedCursor=null,selectedMedia=[],pollTimer=null,globalPoll=null,storageClient=null;
...
"""
new = """let currentTab='feed',currentConversation='',inboxData=[],feedData=[],feedCursor=null,selectedMedia=[],pollTimer=null,storageClient=null,inboxInFlight=null,pollFailures=0,lastInboxAt=0;
...
"""
assert old in s, 'request block not found'
s = s.replace(old, new, 1)

old = """async function loadInbox(silent=false,notificationCheck=false){
...
"""
new = """async function loadInbox(silent=false,notificationCheck=false,refreshConversation=false){
...
"""
assert old in s, 'loadInbox block not found'
s = s.replace(old, new, 1)

old = "currentConversation=id;const data=await request('conversation',{conversation_id:id}),panel=$('#chatPanel');if(!panel)return;"
new = "currentConversation=id;const unreadBefore=Number(inboxData.find(x=>x.id===id)?.unread||0),data=await request('conversation',{conversation_id:id}),panel=$('#chatPanel');if(!panel)return;"
assert old in s, 'openConversation start not found'
s = s.replace(old, new, 1)

old = "const msg=$('#chatMessages');if(msg)msg.scrollTop=msg.scrollHeight;await request('mark_read',{conversation_id:id});const row=inboxData.find(x=>x.id===id);if(row)row.unread=0;updateBadges(totalUnread());renderInbox();"
new = "const msg=$('#chatMessages');if(msg)msg.scrollTop=msg.scrollHeight;if(!silent||unreadBefore>0){await request('mark_read',{conversation_id:id});const row=inboxData.find(x=>x.id===id);if(row)row.unread=0;updateBadges(totalUnread());renderInbox()}"
assert old in s, 'mark_read block not found'
s = s.replace(old, new, 1)
...
"""
new = """function chatActive(){return !document.hidden&&$('#community')?.classList.contains('on')&&currentTab==='chat'}
...
"""
assert old in s, 'polling block not found'
s = s.replace(old, new, 1)
